...
async function saveSearchHistory(query) {
    ...
    try {
        await fetch('https://mini-project-price-pulse.onrender.com/api/history', {
            method: 'POST',
...
async function fetchAndDisplayHistory() {
    ...
    if (!userInfo || !userInfo.token) return;
    try {
        const res = await fetch('https://mini-project-price-pulse.onrender.com/api/history', {
...
async function performSearch(page = 1, saveHistory = true) {
    ...
    try {
        const res = await fetch(`https://mini-project-price-pulse.onrender.com/api/products/compare?product=${encodeURIComponent(query)}&page=${page}`);
