...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (suggestionText) {
        const suggestions = [ 'Try: "wireless headphones", "smartphone", or "laptop"', 'Try: "running shoes", "smartwatch", or "backpack"', 'Try: "bluetooth speaker", "gaming mouse", "monitor"'];
    ...
    // --- STATE PERSISTENCE LOGIC (THE FIX) ---
    const userInfo = JSON.parse(localStorage.getItem('pricePulseUser'));

    if (userInfo && userInfo.token) {
        // 1. If the user is logged in, ALWAYS fetch their history first.
        fetchAndDisplayHistory();

        // 2. Then, check if there was a search query saved in the session.
        const lastSearchQuery = sessionStorage.getItem('lastSearchQuery');
        if (lastSearchQuery) {
            // 3. If a query exists, restore the search bar and run the search.
            console.log("Restoring previous search state for:", lastSearchQuery);
            searchInput.value = lastSearchQuery;
            performSearch(1, false); // Pass 'false' to prevent re-saving the history
        } else {
            // 4. If no query, just show the welcome message.
            showWelcomeMessage();
        }
    } else {
        // 5. If user is not logged in, just show the welcome message.
        showWelcomeMessage();
    }
});
...
async function saveSearchHistory(query) {
    ...
    try {
        ...
        await fetchAndDisplayHistory(); // Refresh history display after saving
    } catch (error) {
...
// Main Search function (Updated with a flag to prevent re-saving)
async function performSearch(page = 1, saveHistory = true) {
    ...
    if (!query) {
        showWelcomeMessage();
        sessionStorage.removeItem('lastSearchQuery'); // Clear session on empty search
        return;
    }

    sessionStorage.setItem('lastSearchQuery', query); // Always save query to session
...
function filterAmazonResponse(data) {
    return (data || []).map((p) => {
      ...
      return {
        id: p?.id, name: p?.name, price: parseFloat(price), originalPrice: parseFloat(originalPrice || "0"),
        rating: parseFloat(p?.rating) || 0, no_of_rating: p?.no_of_rating, image: p?.image, url: p?.url,
        delivery: p?.delivery || "N/A", source: p?.source, reviews: p?.reviews ?? Math.floor(Math.random() * 5000),
...
function filterAndSortResults(products) {
    ...
    if (!products) {
      const cards = Array.from(document.querySelectorAll(".product-card")).map((el) => ({
        element: el, price: parseFloat(el.dataset.price), rating: parseFloat(el.dataset.rating),
        discount: parseFloat(el.dataset.discount), source: el.dataset.source,
      }));
    ...
    products.forEach((product) => {
      ...
      if (product.source.toLowerCase() === 'walmart' && !hasRating) {
          ...
          ratingReviewsHTML = `<div class="rating"><i class="fas fa-star" style="color: ${starColor};"></i> ${product.rating} ${reviewText}</div>`;
      ...
      Object.assign(card.dataset, {
        id: product.id, price: product.price, rating: product.rating,
        discount: discount ?? 0, source: product.source.toLowerCase(),
      });
      card.innerHTML = `
